...
const Header = () => {
  ...
  return (
    <header className="absolute top-2 left-0 w-full text-black z-50 px-4 sm:px-6 md:px-16 lg:px-24">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#FCE67A] text-black flex flex-col items-center justify-center gap-8 text-lg font-medium z-40"
          >
            {["About Us", "Our Services", "Gallery", "Contact Us"].map((label, i) => (
              <a
                ...
                className="hover:text-[#1B2431] transition-colors"
              >
                {label}
              </a>
            ))}
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="mt-4 px-6 py-2 rounded-lg bg-[#1B2431] text-white hover:bg-[#2C3A4A] transition-colors"
            >
              Get Started
            </button>
          </motion.div>
